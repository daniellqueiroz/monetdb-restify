...
function connect() {
	...
	connection = new MonetDB(connectionOptions);

	connection.option("prettyResult", true);
	
	connection.connect().fail(function(err) {
		console.log(err);
	});
...
function filterToString(array) {
	...
	var query_string = filter + ' ' + orderBy + ' ' + groupBy + ' ' + limit + ' ' + offset;
...
MonetDBInterface.prototype.select = function(schema, table, options) {
	...
	if (!options.filter) {
		...
		return conn.query('SELECT ' + columns + ' FROM ' + schema + '.' + table);
	}
	var filter = filterToString(options.filter);
	return conn.query('SELECT ' + columns + ' FROM ' + schema + '.' + table + ' ' + filter.query_string, filter.values);
};
...
MonetDBInterface.prototype.insert = function(schema, table, options) {
	...
	if (!options.columns) {
		return conn.query('INSERT INTO ' + schema + '.' + table +
...
MonetDBInterface.prototype.update = function(schema, table, options) {
	...
	if (!options.filter) {
		return conn.query('UPDATE ' + schema + '.' + table +
...
MonetDBInterface.prototype.deleteRow = function(schema, table, options) {
	...
	if (!options.filter) {
		return conn.query('DELETE FROM ' + schema + '.' + table);
	}
	var filter = filterToString(options.filter);
	return conn.query('DELETE FROM ' + schema + '.' + table +
			' ' + filter.query_string,
...
MonetDBInterface.prototype.createTable = function(schema, table, options) {
	...
	return conn.query('CREATE TABLE ' + schema + '.' + table + ' ( ' + options.columns + ' )');
};

MonetDBInterface.prototype.dropTable = function(schema, table) {
	var conn = connect();

	return conn.query('DROP TABLE ' + schema + '.' + table);
};

MonetDBInterface.prototype.createView = function(schema, view, options) {
	var conn = connect();

	return conn.query('CREATE VIEW ' + schema + '.' + view + ' AS ' + options.query);
};

MonetDBInterface.prototype.dropView = function(schema, view) {
	var conn = connect();

	return conn.query('DROP VIEW ' + schema + '.' + view);
};
...
MonetDBInterface.prototype.executeFunction = function(name, params) {
	...
	if (params.length > 0) {
		...
		return conn.query('CALL ' + name + ' ( ' + parameters + ' )');
	}
	return conn.query('CALL ' + name + '()');
};

MonetDBInterface.prototype.getAllSchemas = function() {
	return this.select('sys', 'schemas', {
		columns : 	['name']
	});
};

MonetDBInterface.prototype.getAllTables = function(schema, type) {
	return this.executeQuery('SELECT name from sys.tables ' +
...
MonetDBInterface.prototype.getAllColumns = function(schema, table) {
	return this.executeQuery('SELECT name, type FROM sys.columns ' +
